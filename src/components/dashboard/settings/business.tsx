...
import { UpdateBusinessDetails } from "@/actions/dashboard/settings/business";
...
export default function BusinessSetting({
  ...
  return (
    <div className="w-full h-full">
      ...
      <div className="w-full h-full px-2 flex flex-col items-center justify-start md:flex-row md:justify-between md:items-start">
        <BusinessLogoUpdate
          id={updatedInfo?.id}
          image={updatedInfo?.image || ""}
        />
        <form action={formAction} className="w-full h-full space-y-2 m-4">
          <h1 className="mb-4 text-xl">Business Details</h1>
          <div className="relative">
            ...
            >
              {updatedInfo?.profession && (
                <option>{updatedInfo.profession}</option>
              )}
              <option value="Engineer">Engineer</option>
              <option value="Chemist">Artist</option>
              <option value="Dentist">Dentist</option>
              <option value="Chemist">Doctor</option>
              <option value="Chemist">Musician</option>
              <option value="Chemist">Teacher</option>
              <option value="Chemist">Farmer</option>
              <option value="Chemist">Chemist</option>
              <option value="Chemist">Tailor</option>
            </select>
            <label
              htmlFor="profession"
          ...
          <textarea
            ...
            name="bio"
            rows={4}
            className="block p-2.5 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer"
            placeholder="Your logo will be displayed on the top left of the booking portal, and all customer notifications (optional)"
          />
